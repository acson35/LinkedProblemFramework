...
import main.algorithm.SequentialAlgorithm.SequentialAlgorithms;
...
public class RunSequenceAlgorithmsIwspVapMtsp<S extends solution<?>> {
	
	
	public RunSequenceAlgorithmsIwspVapMtsp() {
	...
	public List<Algorithm<S, S>> getAlgorithms(Problem parentproblem1, Problem parentproblem2, Problem childproblem, double pc1, double mc1, double pc2, int distributionIndex, double pc3, double mc3){
		...
		Algorithm<?, ?> algorithm1 = new IwspGurobiAlgorithm(parentproblem1);
		algorithmList.add((Algorithm<S, S>) algorithm1);
		//int numberOfCores = 4;
	    Algorithm<IntegerSolution, IntegerSolution> algorithm2 = BuildAlgorithm(parentproblem2, 5000, 50,
	    		new IntegerSBXCrossover(pc2, distributionIndex),  new IntegerPolynomialMutation(),
	    		new TournamentSelection<IntegerSolution>(new ObjectiveComparator(), 25), 
	    		new  MultiThreadedSolutionListEvaluator<IntegerSolution>(10), 
	    		new ObjectiveComparator(), 
	    		GeneticAlgorithmVariant.GENERATIONAL);
	    
	    algorithmList.add((Algorithm<S, S>) algorithm2);
	      
	    Algorithm<IntegerPermutationSolution, IntegerPermutationSolution> algorithm3 = BuildAlgorithm(childproblem, 5000, 50,
	    		new PMXCrossover(pc3),  new PermutationSwapMutation(mc3),
	    		new TournamentSelection<IntegerPermutationSolution>(new ObjectiveComparator(), 25), 
	    		new  MultiThreadedSolutionListEvaluator<IntegerPermutationSolution>(10), 
	    		new ObjectiveComparator(), 
	    		GeneticAlgorithmVariant.GENERATIONAL);  
	   
	    algorithmList.add((Algorithm<S, S>) algorithm3);
		   
		return algorithmList;
	}
	
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public void SEQUENCE_IwspVapMtspExperiments(String[] args, String[]args2) throws IOException {
    ...
    for(int i = 0; i < args.length; i++) {
		...
		experiment = new LinkedProblemRunnerBuilder("SequenceBased_"+parentproblem.getName()+"_"+childproblem.getName()+"_"+ childproblem1.getName()+"_"+args2[i])
			       .setProblemList(linkedproblem)
			       .setAlgorithmList(getAlgorithms(parentproblem, childproblem,childproblem1,pc1,mc1,pc1,distIndex,pc1,mc1))
			       .setOutputLinkedProblemFileName("")
			       .setIndependentRuns(1)
			       .build();   
    	
    	FileOutputContext funContext = new DefaultFileOutputContext("C:/Users/akinola/OneDrive - Robert Gordon University/Documents/Datapipe/OutputData/SEQ_Fitness"+p1name+p2name+p3name+"/"+experiment.getOutputLinkedProblemFileName()+".txt", ",");
        FileOutputContext varContext = new DefaultFileOutputContext("C:/Users/akinola/OneDrive - Robert Gordon University/Documents/Datapipe/OutputData/SEQ_Variables"+p1name+p2name+p3name+"/"+experiment.getOutputLinkedProblemFileName()+".txt", ",");
    	...
    	for(int j=0; j<20;j++) {
    		...
    		SequentialAlgorithms<S, solutionSet> sequentialAlgo =  new SequentialAlgorithms<S, solutionSet>(experiment);
    		
    		sequentialAlgo.run();
    		
    		
			solutionSet sol2 = sequentialAlgo.getResult();
    		...
    		**/          	
        	try {
        		...
        		myprintObjectivesToFile(linkedproblem, funbufferedWriter, sol2, compTime, pc1, mc1);
	...
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public void myprintObjectivesToFile(IWSP_VAP_MTSP_Problems pro, BufferedWriter bufferedWriter, solutionSet solutionpair, long computingTime, double pc1, double mc1) {
			  try {
			    if (null != solutionpair) {
			    				    	
			    	IWSP iwsp = (IWSP) pro.getLinkedProblems().get(1);
			    	VAP vap = (VAP) pro.getLinkedProblems().get(2);
