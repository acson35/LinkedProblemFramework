...
public class Run_MCRGALP_SFLP_SPFSP_Experiments<S extends solution<?>> {

	public Run_MCRGALP_SFLP_SPFSP_Experiments() {
	...
	public MCRGALPs getAlgorithms(LinkedProblem linkedproblem, double pc1, double mc1, double pc2, double mc2, SelectionOperator<List<solutionSet>, List<solutionSet>> selectionOperator, List<Criteria> criteria){
		...
		/**	List<Criteria> criteria = new ArrayList<>();
			...
			**/
			//SelectionOperator<List<solutionSet>, List<solutionSet>> so = selectionOperator;
	...
	public void MCRGALP_SFLP_SPFSP_Experiments
		(String method, SelectionOperator<List<solutionSet>, List<solutionSet>> selectionOperator, List<Criteria> criteria, String[] args_sflp, String[] args2_sflp, String[] args_spfsp, String[] args2_spfsp) throws IOException{
		
		
		 for(int i = 0; i < args_sflp.length; i++) {
			 
			 String sflp_filename = args_sflp[i].toString();
			 ...
			 for(int j = 0; j < args_spfsp.length; j++) {
				 ...
				 String spfsp_filename = args_spfsp[i].toString();
				 
				 SFLP_SPFSP_Problems linkedproblem = new SFLP_SPFSP_Problems(sflp_filename, spfsp_filename);
	        		
				 SFLP parentproblem = (SFLP) linkedproblem.getLinkedProblems().get(1);
				 int problemsize1 = ((SFLPInstance) parentproblem.getData()).getnCustomers();
	        		
				 SPFSP childproblem = (SPFSP) linkedproblem.getLinkedProblems().get(2);
				 ...
				 if(problemsize1==problemsize2) {	
					 ...
					 LinkedProblemRunner experiment;
			        	...
			        	FileOutputContext funContext = new DefaultFileOutputContext("C:/Users/akinola/OneDrive - Robert Gordon University/Documents/Datapipe/OutputData/MCRGALP_" + method +"_Fitness_"+p1name+p2name+"/"+experiment.getOutputLinkedProblemFileName()+".txt", ",");
			            FileOutputContext varContext = new DefaultFileOutputContext("C:/Users/akinola/OneDrive - Robert Gordon University/Documents/Datapipe/OutputData/MCRGALP_" + method +"_Var_"+p1name+p2name+"/"+experiment.getOutputLinkedProblemFileName()+".txt", ",");
