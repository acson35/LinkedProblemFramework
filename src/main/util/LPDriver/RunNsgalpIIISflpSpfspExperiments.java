...
public class RunNsgalpIIISflpSpfspExperiments<S extends solution<?>> {

	

	public RunNsgalpIIISflpSpfspExperiments() {
		
	}
	
	
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public NSGALPIII getAlgorithms(LinkedProblem linkedproblem, double pc1, double mc1, double pc2, double mc2) {
		//int numberOfCores = 4;
        ...
        for(int i = 0; i < args_sflp.length; i++) {
        	...
        	SFLP parentproblem = new SFLP(sflp_filename);
		 	...
		 	if(nfacilities > 30)continue;
   			...
   			// System.out.println(args2_spfsp[j]);
   		 		...
   		 		if(problemsize1==problemsize2) {
   		 			...
   		 			experiment = new LinkedProblemRunnerBuilder("NSGALP_III_" + parentproblem.getName()+"_"+childproblem.getName()+"_"+args2_sflp[i].replace(".txt", "")+"_"+ args2_spfsp[j].replace(".txt", ""))
	    			       .setProblemList(linkedproblem)
	    			       .setOutputLinkedProblemFileName("")
	    			       .setIndependentRuns(1)
	    			       .build(); 
	        	
   		 			FileOutputContext funContext = new DefaultFileOutputContext("C:/Users/akinola/OneDrive - Robert Gordon University/Documents/Datapipe/OutputData/NSGALP_III_Fitness_"+p1name+"_"+p2name+"/"+experiment.getOutputLinkedProblemFileName()+".txt", ",");
   		 			FileOutputContext varContext = new DefaultFileOutputContext("C:/Users/akinola/OneDrive - Robert Gordon University/Documents/Datapipe/OutputData/NSGALP_III_Var_"+p1name+"_"+p2name+"/"+experiment.getOutputLinkedProblemFileName()+".txt", ",");
	          ...
	          //NSGALP_II_Fitness_SFLP_SPFSP
	        	...
	        	for(int k=0; k<10;k++) {
	        		...
	        		NSGALPIII NSGALP_Algo = (NSGALPIII)getAlgorithms(linkedproblem,pc1,mc1,pc1,mc1);
	...
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public void printObjectivesToFiles(NSGALPIII proAlgo, BufferedWriter bufferedWriter, List<solutionSet> solutionSets, long computingTime, double pc1, double mc1) {
			  try {
